...
export default function WaterButton(props: WaterButtonProps) {
  ...
  useEffect(() => {
    ...
    const surfaceTone = `rgba(${shift(cr, 0.45)}, ${shift(cg, 0.45)}, ${shift(
      cb,
      0.45,
    )}, ${ca})`;
    const bodyTone = `rgba(${cr}, ${cg}, ${cb}, ${ca})`;
    const depthTone = `rgba(${shift(cr, -0.35)}, ${shift(cg, -0.35)}, ${shift(
      cb,
      -0.35,
    )}, ${ca})`;
    const glintTone = `rgba(${shift(cr, 0.75)}, ${shift(cg, 0.75)}, ${shift(
      cb,
      0.75,
    )}, ${Math.min(1, ca + 0.25)})`;
    ...
    function spray(dt: number) {
      ...
      for (let i = 1; i < COLUMNS - 1; i++) {
        ...
        if (Math.random() > (flow - BREAK_SPEED) * 0.004 * ripple * dt * 60) {
          continue;
        }
  ...
  }, [waterAmount, waterColor, border, borderColor, borderStroke, rounded]);
